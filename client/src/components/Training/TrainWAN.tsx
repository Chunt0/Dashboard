...
const TrainWAN: React.FC = () => {
        ...
        const handleTrainWANModel = async () => {
                try {
			const response = await fetchWithAuth(TRAIN_WAN_MODEL_ENDPOINT, {
				method: 'POST',
				headers: {
	...
	return (
		<div className="mx-auto flex min-h-[70vh] w-full max-w-xl items-center justify-center">
			<div className="w-full border border-slate-800 bg-slate-950 p-8 text-center">
				<p className="text-xs font-medium text-slate-400">WAN Training</p>
				<h2 className="mt-3 text-2xl font-semibold text-slate-100">Select a dataset to train.</h2>
				<p className="mt-3 text-sm text-slate-300">Launch a WAN2.1 LoRA run from a completed dataset.</p>
				<select
